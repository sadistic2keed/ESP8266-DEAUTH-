...
void showHelp() {
    ...
    Serial.println("  scanoff     - Stop scanning");
    Serial.println("  deauth X    - Start deauth attack on network X (X = Index Number)");
    Serial.println("  deauthoff   - Stop deauth attack");
...
void loop() {
    if (Serial.available()) {
        ...
        else if (command.equalsIgnoreCase("scanoff")) {
            if (!scanExecuted) {
        ...
        else if (command.equalsIgnoreCase("deauthoff")) {
            if (!deauthExecuted) {
